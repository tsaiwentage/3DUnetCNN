# 5层深度，其余什么都没有，学习率5e-4，加att，res+dropout，leaky relu
import os
import glob

from unet3d.data import write_data_to_file, open_data_file
from unet3d.generator import get_training_and_validation_generators
from unet3d.model import isensee_3
...
config["initial_learning_rate"] = 5e-4  # 0.00001  # 5e-4
...
def main(overwrite=False):
    ...
    if not overwrite and os.path.exists(config["model_file"]):
        ...
        model = isensee_3.isensee_3_model(input_shape=config["input_shape"], n_labels=config["n_labels"],
                                          initial_learning_rate=config["initial_learning_rate"],
                                          n_base_filters=config["n_base_filters"],
                                          depth=5)
        plot_model(model, to_file='isensee_3_uet.png', show_shapes=True)
